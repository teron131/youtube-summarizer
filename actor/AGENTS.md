# YouTube Summarizer Actor

AI-powered YouTube video analysis with transcript extraction and comprehensive summarization using LangGraph.

## Actor-Specific Information

### Environment Variables Required

This Actor requires the following environment variables to be set:

- `SCRAPECREATORS_API_KEY` - API key for Scrape Creators YouTube API (https://scrapecreators.com)
- `OPENROUTER_API_KEY` - API key for OpenRouter LLM access (https://openrouter.ai)

Set these in the Apify Console under Actor Settings > Environment variables, or locally in `.env` file.

### How This Actor Works

**Input:**
- **youtubeUrls** (required): Array of YouTube video URLs to analyze
- **analysisModel** (optional): LLM model for content analysis (default: `x-ai/grok-4.1-fast`)
- **qualityModel** (optional): LLM model for quality assessment (default: `x-ai/grok-4.1-fast`)
- **targetLanguage** (optional): ISO language code for output (default: `en`)
- **maxVideos** (optional): Maximum number of videos to process (default: 10, max: 100)

**Processing Workflow:**
1. **Video Scraping**: Uses Scrape Creators API to extract video metadata and transcript
2. **LangGraph Analysis**: Self-checking AI workflow with quality assessment
   - Generates comprehensive analysis (title, summary, takeaways, chapters, keywords)
   - Evaluates quality across 6 aspects (completeness, structure, no_garbage, meta_language_avoidance, useful_keywords, correct_language)
   - Iteratively refines if quality < 80% (max 2 iterations)
3. **Output**: Structured results pushed to dataset

**Output Format:**
Each analyzed video produces a dataset item with: url, title, summary, takeaways, chapters, keywords, quality_score, iteration_count, processing_time, video_author, video_duration, processed_at
